...
void Image::pad_kernel(uint32_t ker_w, uint32_t ker_h, double ker[],
                       uint32_t cr, uint32_t cc, uint32_t pw, uint32_t ph,
  ...
  for (long i = -((long)cr); i < (long)ker_h - cr; ++i) {
    ...
    for (long j = -((long)cc); j < (long)ker_w - cc; ++j) {
      ...
      pad_ker[r * pw + c] =
          std::complex<double>(ker[(i + cr) * ker_w + (j + cc)], 0);
    }
  }
}
void Image::pointwise_product(uint64_t l, std::complex<double> a[],
...
Image& Image::fd_convolve_clamp_to_0(uint8_t channel, uint32_t ker_w,
                                     uint32_t ker_h, double ker[], uint32_t cr,
                                     uint32_t cc) {
  // calculate padding
  ...
  for (uint32_t i = 0; i < h; ++i) {
    for (uint32_t j = 0; j < w; ++j) {
      pad_img[i * pw + j] =
          std::complex<double>(data[(i * w + j) * channels + channel], 0);
    }
  }
  ...
  for (uint32_t i = 0; i < h; ++i) {
    for (uint32_t j = 0; j < w; ++j) {
      data[(i * w + j) * channels + channel] =
          BYTE_BOUND((uint8_t)round(pad_img[i * pw + j].real()));
    }
  }

  return *this;
}
Image& Image::fd_convolve_clamp_to_border(uint8_t channel, uint32_t ker_w,
                                          uint32_t ker_h, double ker[],
                                          uint32_t cr, uint32_t cc) {
  // calculate padding
  uint32_t pw = 1 << ((uint8_t)ceil(log2(w + ker_w - 1)));
  ...
  for (uint32_t i = 0; i < ph; ++i) {
    ...
    for (uint32_t j = 0; j < pw; ++j) {
      ...
      pad_img[i * pw + j] =
          std::complex<double>(data[(r * w + c) * channels + channel], 0);
    }
  }
...
Image& Image::fd_convolve_cyclic(uint8_t channel, uint32_t ker_w,
                                 uint32_t ker_h, double ker[], uint32_t cr,
                                 uint32_t cc) {
  // calculate padding
  uint32_t pw = 1 << ((uint8_t)ceil(log2(w + ker_w - 1)));
    ...
    for (uint32_t j = 0; j < compare_width; ++j) {
      for (uint8_t k = 0; k < compare_channels; ++k) {
        data[(i * w + j) * channels + k] =
            BYTE_BOUND(abs(data[(i * w + j) * channels + k] -
...
Image& Image::encodeMessage(const char* message) {
  ...
  if (len + STEG_HEADER_SIZE > size) {
    printf(
        "\e[31m[ERROR] This message is too large (%lu bits / %zu bits)\e[0m\n",
...
Image& Image::overlayText(const char* txt, const Font& font, int x, int y,
...
Image& Image::crop(uint16_t cx, uint16_t cy, uint16_t cw, uint16_t ch) {
  ...
  for (uint16_t y = 0; y < ch; ++y) {
    ...
    for (uint16_t x = 0; x < cw; ++x) {
      ...
      memcpy(&croppedImage[(x + y * cw) * channels],
...
Image& Image::resizeNN(uint16_t nw, uint16_t nh) {
  ...
  for (uint16_t y = 0; y < nh; ++y) {
    ...
    for (uint16_t x = 0; x < nw; ++x) {
      ...
      memcpy(&newImage[(x + y * nw) * channels],
             &data[(sx + sy * w) * channels], channels);
    }
  }
